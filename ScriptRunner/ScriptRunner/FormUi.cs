...
namespace ScriptRunner
{
    public partial class FormUi : Form
    {
        ...
        {
            string lastTmpDir = Properties.Settings.Default.LastTmpDir;

            var dirDialog = new FolderBrowserDialog();
            dirDialog.Description = "Select temp directory";
            dirDialog.ShowNewFolderButton = true;
            if (!string.IsNullOrEmpty(lastTmpDir) && Directory.Exists(lastTmpDir))
                dirDialog.SelectedPath = lastTmpDir;
            ...
            if (dirDialog.ShowDialog() == DialogResult.OK && Directory.Exists(dirDialog.SelectedPath))
            {
                ...
                AllowRunScript();
                Properties.Settings.Default.LastTmpDir = dirDialog.SelectedPath;
